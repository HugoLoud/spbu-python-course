...
def non_decreasing_index(generator_func: Callable[[], Generator[T, None, None]]) -> Callable[[int], T]:
    """
    ...
    def wrapper() -> Callable[[int], T]:
        ...
        def inner(k: int) -> T:
            nonlocal last_k
            if k < last_k:
                raise ValueError(f"Index {k} is less than the last requested index {last_k}. "
