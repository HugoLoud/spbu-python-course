...
def sum_matrix(a: Optional[List[List[int]]], b: Optional[List[List[int]]]) -> List[List[int]]:
    ...
    if get_row_matrix(a) != get_row_matrix(b) or get_cols_matrix(a) != get_cols_matrix(b):
        raise ValueError("Размеры матриц не совпадают")

    # Суммируем соответствующие элементы обеих матриц
    return [[a[i][j] + b[i][j] for j in range(get_cols_matrix(a))] for i in range(get_row_matrix(a))]
